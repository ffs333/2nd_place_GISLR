...
def timeSince(since, percent):
    ...
    es = s / (percent)
    rs = es - s
    return str(datetime.timedelta(seconds = rs))[:-7]
...
class ExperimentWriter:
    ...
    NAME_METRICS_LOG = "metrics.log"
    NAME_METRICS_JSON = "metrics.json"
    ...
    def __init__(self, log_dir: str) -> None:
        ...
        self.metrics_file_log = os.path.join(self.log_dir, self.NAME_METRICS_LOG)
        self.metrics_file_json = os.path.join(self.log_dir, self.NAME_METRICS_JSON)
    ...
    def save(self) -> None:
        ...
        with open(self.metrics_file_json, "a" if self.json_saved else "w", newline="") as f:
            self.json_saved = True
            f.write(json.dumps(self.metrics[-1]) + "\n")
        with open(self.metrics_file_log, "a" if self.log_saved else "w", newline="") as f:
            self.log_saved = True
            met = self.metrics[-1]
            if "train_loss" in met:
                other_met = {k: v for k, v in met.items() if k not in {"epoch", "step", "total_steps", "num_epochs"}}
                format_log = "Epoch: [{0}][{1}/{2}]\teta: {remain:s}"
                remain = timeSince(self.start_time, float(met["step"] + 1) / (met["total_steps"] * met["num_epochs"]))
                for k in other_met:
                    format_log += ", " + k + ": {" + k + ":.4f}"
                step = (met["step"] + 1) % int(met["total_steps"])
                if step == 0: step = int(met["total_steps"])
                format_log = format_log.format(met["epoch"] + 1, 
                                               step, 
                                               int(met["total_steps"]),
                                               remain = remain,
                                               **other_met)
                ...
                other_met = {k: v for k, v in met.items() if k not in {"epoch", "step", "total_steps", "num_epochs"}}
                format_log = "Epoch(val): [{0}]\t"
                for i, k in enumerate(other_met):
                    format_log += (", " if i != 0 else "") + k + ": {" + k + ":.4f}"
                format_log = format_log.format(met["epoch"] + 1,
                                               **other_met)
            f.write(format_log + "\n")
