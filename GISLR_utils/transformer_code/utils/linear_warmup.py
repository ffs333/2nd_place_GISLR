...
class LinearWarmupCosineAnnealingLR(_LRScheduler):
    ...
    def get_lr(self) -> List[float]:
        ...
        if self.last_epoch == self.warmup_epochs or self.last_epoch <= self.plateau_epochs:
            return self.base_lrs
        if (self.last_epoch - 1 - self.max_epochs) % (2 * (self.max_epochs - self.plateau_epochs)) == 0:
            return [
                group["lr"]
                + (base_lr - self.eta_min) * (1 - math.cos(math.pi / (self.max_epochs - self.plateau_epochs))) / 2
                for base_lr, group in zip(self.base_lrs, self.optimizer.param_groups)
            ]

        return [
            (1 + math.cos(math.pi * (self.last_epoch - self.plateau_epochs) / (self.max_epochs - self.plateau_epochs)))
            / (
                1
                + math.cos(
                    math.pi * (self.last_epoch - self.plateau_epochs - 1) / (self.max_epochs - self.plateau_epochs)
                )
            )
    ...
    def _get_closed_form_lr(self) -> List[float]:
        ...
        if self.last_epoch < self.plateau_epochs:
            return self.base_lrs
