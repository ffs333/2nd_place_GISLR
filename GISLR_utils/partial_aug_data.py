...
def random_slices(len_, a_min, a_max):
    slices = []
    start = 0

    while start < len_:
        slice_len = np.random.randint(a_min, a_max + 1)
        end = start + slice_len
...
class DatasetImageSmall80Partial(torch.utils.data.Dataset):
    def __init__(self, cfg, df, Y, train_mode=True):
        ...
        self.use_slices_prob = cfg.use_slices_prob
    ...
    def rotate_aug(self, img, min_angle=-10, max_angle=10):
    ...
    def get_one_item(self, index):
        ...
        if self.train_mode:
            ...
            if random.random() < self.shift_prob:
                if yy.shape[0] > 5 and random.random() < self.use_slices_prob:
                    slices = random_slices(yy.shape[0], 1, len(yy))
                    for sl in slices:
                        yy[sl[0]:sl[1]] = self.shift_aug(yy[sl[0]:sl[1]])
                else:
            ...
            if random.random() < self.scale_prob:
                if yy.shape[0] > 5 and random.random() < self.use_slices_prob:
                    slices = random_slices(yy.shape[0], 1, len(yy))
                    for sl in slices:
                        yy[sl[0]:sl[1]] = scale_parts(yy[sl[0]:sl[1]])
                else:
                    yy = scale_parts(yy)
                ...
                if yy.shape[0] > 5 and random.random() < self.use_slices_prob:
                    ...
                    for sl in slices:
                        if random.random() < 0.75:
                            yy[sl[0]:sl[1]] = self.rotate_aug(yy[sl[0]:sl[1]],
                                                              min_angle=-7, max_angle=7)
                else:
            ...
            if random.random() < self.tree_rot_prob:
                if yy.shape[0] > 5 and random.random() < self.use_slices_prob:
                    ...
                    for sl in slices:
                        if random.random() < 0.75:
                            yy[sl[0]:sl[1], self.l_hand] = self.random_hand_rotate(yy[sl[0]:sl[1], self.l_hand].numpy(),
                                                                                   degree=(-3, 3),
                                                                                   joint_prob=0.5)
                else:
            ...
            if random.random() < self.tree_rot_prob:
                if yy.shape[0] > 5 and random.random() < self.use_slices_prob:
                    ...
                    for sl in slices:
                        if random.random() < 0.75:
                            yy[sl[0]:sl[1], self.r_hand] = self.random_hand_rotate(yy[sl[0]:sl[1], self.r_hand].numpy(),
                                                                                   degree=(-3, 3),
                                                                                   joint_prob=0.5)
    ...
    def __getitem__(self, i):
        ...
        if random.random() < self.mixup_prob and self.train_mode:
            ...
            mix_lambda = np.random.beta(0.3, 0.3)
